...
@section('content')
     ...
     <div class="row widget-bg-color-white no-space margin-bottom-20">
              <form role="form" id="skills" name="skills">
                    <div class="col-md-3 col-sm-6 no-space">
                        <!-- BEGIN WIDGET SUBSCRIBE -->
                        	<div class="portlet-body form">
                                    <input type="hidden" id="id" name="id" v-model="skill.id" value="{{ $skill->id }}"></input>
                                  
                                    <div class="form-body">
                                        <div class="form-group form-md-line-input">
                                            <input v-on:blur="SaveSkillDetail('name', skill.name , $event )" type="text" class="form-control" v-model="@{{ skill.name }}" value="{{ $skill->name }}" id="name" name="name" placeholder="Enter your name">
                                        ...
                                        <div class="form-group form-md-line-input">
                                            <textarea class="form-control" id="description" v-model="@{{ skill.description }}" name="description" rows="3" v-on:blur="SaveSkillDetail('description', skill.description , $event )">{{ $skill->description }}</textarea>
                    ...
                    <div class="col-md-3 col-sm-6 no-space">
                        <!-- BEGIN WIDGET SUBSCRIBE -->
                       	     <div id="preview" style="background: url(@{{ skill.photo  | check_photo }} ) no-repeat" class="widget-gradient margin-top-20">
                        </div>
...
<script>
/*    sv.skill.photo = '{!! $skill->photo !!}';
    sv.skill.name = '{!! $skill->name !!}';
    sv.skill.description = '{!! $skill->description !!}';
    sv.skill.user_id = '{!! $skill->user_id !!}';
