                    ...
                    <div class="col-md-3 col-sm-6 no-space">
                        ...
                        <div class="form-body">
                            <div class="form-group">
                                       ...
                                       <div id="preview" style="background: url({!! $project->photo !!});  background-repeat: no-repeat;" class="widget-gradient margin-top-20">
                                    </div>
                     ...
                     <div class="col-md-3 col-sm-3 no-space">
                        ...
                        <div class="portlet-body form ">
                            <button class="btn btn-primary" v-on:click="AddNewCredential()" type="button">New</button >                            
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="icheck-inline">
                                            ...
                                            <div v-for="company in managingCompanies" style="float: left;">                                      
                                                <label>
                                                <input @click="SaveProjectManager(project.id,  company.id, 0,$event)" type="checkbox" id="company_id_@{{ company.id }}" name="company_id_@{{ company.id }}" class="project-manager company icheck" v-model="company.selected" > @{{ company.name}} </label>
                                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="icheck-inline">
                                                <div class="row">
                                                    <div>Admin</div>
                                                    <div v-for="admin in managingAdmins" style="float: left;">
                                                        <label>
                                                        <input type="checkbox" id="user_id_@{{ admin.id }}" name="user_id_@{{ admin.id }}" value=@{{ admin.id }} class="project-manager user icheck"  @click="SaveProjectManager(project.id,  0, admin.id,$event)"  v-model="admin.selected"> @{{ admin.name}}</label>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row">
                                                       ...
                                                       <div v-for="client in managingClients" style="float: left;">
                                                            <label>
                                                            <input  type="checkbox" id="user_id_@{{ client.id }}" name="user_id_@{{ client.id }}" @click="SaveProjectManager(project.id,  0, client.id,$event)" class="project-manager user icheck" value="@{{ client.id }}" v-model="client.selected" />   @{{ client.name}} </label>
                                                        </div>
                                                <div>
...
<!--      
    <template id="clients-cb-template">
        test
        <div v-for="client in clients" style="float: left;">
            <label>
            <input  type="checkbox" id="user_id_@{{ client.id }}" name="user_id_@{{ client.id }}" @click="SaveProjectManager(project.id,  0, client.id,$event)" class="project-manager user icheck" value="@{{ client.id }}" v-model="true">  @{{ client.first_name}} @{{ client.last_name}} </label>
        </div>
    </template>
 -->
@stop
...
var mixin = {
    ...
    ready : function(){
            ...
            this.FetchClients();
            this.FetchAdmins();
            this.FetchProjectManagingUsers();
            this.FetchProjectManagingCompanies();
    }
